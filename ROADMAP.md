...
> 实现：`pipeline/langgraph/`（编排）+ `pipeline/stage_executors.py`（节点实现，与 workflow 解耦不冲突）。**三路线覆盖**：A 骨骼（S0 生图→S1 拆层→S2 绑骨→S3 动画→S4 打包→S5 引擎，全接真实工具）、B 关键帧（kb1 gen-frame-cycle→kb2 Godot 帧动画）、F 3D（f1-f5，后续补充）；门禁 FAIL 自动沉淀经验并回退重试；可观察/可审计/可断点续跑。
> 进度：L0-L3.6 ✅（A1 + 三路线：S0-S5 骨骼 + kb1-kb2 关键帧 + f1-f5 3D 桩 + 实现层解耦，离线冒烟通过）；L4-L6 📋（真实跑通/可观察性/workbench 接线）
