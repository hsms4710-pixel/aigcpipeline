...
> 状态：**规划中（暂不开发，Part 3）** ｜ 对应 ROADMAP Part 3 ｜ 选型基线见 TECH-STACK.md
...
## 2. 引擎采用事实（为什么 Godot 是主实现、但不是唯一）
- 工业整体：Unity（移动/indie 主导，GMTK 2025 参赛 64%）+ Unreal（主机/3A）占主导；Godot 增长最快（20%，indie 2D 尤其强）且开源无授权费
- **结论**：Godot 作为主实现引擎（简单/开源/2D 强），但 P1 资产 + P3 Agent 必须引擎无关 → Unity/UE 只是"导入目标"（OpenGameAgent 已证明双引擎可行）

## 3. 契约
- NPC 实体 = P1 资产（立绘/分层/语音）+ P3 Agent 连接配置
- 事件总线：游戏事件（靠近/对话/任务）→ Agent；Agent 动作 → 白名单执行器
- 白名单动作示例：say / emote / face_player / move_to(范围) / give_item(白名单) / set_quest_flag

## 4. 开源/论文/产品借鉴清单
### Godot
- **noko**（Godot 插件，Ollama API，动态对话/智能 NPC）→ 最直接参照
- **godot-AI-Dialog**（免费 AI 对话生成，RPG 动态任务）→ 对话 UI 参照
- **OpenGameAgent**：Godot 4.7 .NET + Unity 6，agent kernel + 游戏感知原语 → 双引擎架构参照
- **openagentic-sdk-gdscript**（Godot 4.x Agent SDK）、Player2 AI NPC（asset lib）

### 接入现有游戏（Mod，工业与社区均有）
- **StardewLivingNPCs**（星露谷 SMAPI）：AI NPC + 白名单安全动作 + SVE 进度感知层 + **离线回归检查** → 模式 B 最佳参照
...
- **Inworld / Charisma / ego AI / NVIDIA ACE**：商业平台形态参照（见 P3 spec）
